    ...
    switch ( SETUP_TYPE ) {
        ...
        case 0x40:
            ...
            for (c=0;c<8;++c) {
                ...
                buf[0] = EP0BUF[c];
                i2c_write ( PROM_ADDR, 2, i2c_addr_buf, 2, buf); 
            }
            return TRUE;
       case 0xc0:
            ...
            for (c=0;c<8;++c) {
                i2c_addr_buf[0] = MSB(PROM_SERIAL_OFFSET+c*2);
                i2c_addr_buf[1] = LSB(PROM_SERIAL_OFFSET+c*2);
                i2c_write ( PROM_ADDR, 2, i2c_addr_buf, 0, NULL );
                i2c_read ( PROM_ADDR, 1, EP0BUF+c );
            }
            EP0BCH=0; SYNCDELAY();
            EP0BCL=8;
            return TRUE;
...
void main_init() {
 ...
 if (EEPROM_TWO_BYTE) { // this will fail anyway if a two byte prom wasn't detected
    ...
    i2c_write( PROM_ADDR, 2, i2c_addr_buf, 0, NULL );
    i2c_read ( PROM_ADDR, 16, (xdata BYTE*)&str_serial + 2 ); 
 }
